...
class NVI(Indicator):
    """Negative Volume Index — price change on decreasing volume days."""
    _ind_name   = "NVI"
    _key_params = ()
    def init_depends(self): pass
    def __init__(self):
        super().__init__(value_extractor=None)
        self._nvi = 1000.0; self._prev_close = self._prev_vol = None
    def add_input_value(self, raw) -> None:
        ...
        if self._prev_close is not None and self._prev_vol is not None:
            if raw.volume < self._prev_vol and self._prev_close:
                self._nvi += (raw.close - self._prev_close) / self._prev_close * self._nvi
        self._prev_close = raw.close; self._prev_vol = raw.volume
        self._pipe.emit(self._nvi)
    def _first_calculate(self, v, prev): return None
    def _calculate_new_value(self, v, prev): return None
    def get_state(self) -> dict:
        s = super().get_state()
        if s: s["nvi"] = self._nvi; s["prev_close"] = self._prev_close; s["prev_vol"] = self._prev_vol
        return s
    def set_state(self, state: dict) -> None:
        super().set_state(state)
        if state: self._nvi = state.get("nvi", 1000.0); self._prev_close = state.get("prev_close"); self._prev_vol = state.get("prev_vol")
